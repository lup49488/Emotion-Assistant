...
test('chat header controls set a gentle tone and open the model reply window', async ({ page }) => {
  ...
  await settings.getByRole('button', { name: /DeepSeek/ }).click()
  await expect(settings.getByRole('button', { name: 'deepseek-flash', exact: true })).toHaveClass(/selected/)
  await expect(settings.getByRole('button', { name: 'deepseek-v4-pro', exact: true })).toBeVisible()
  ...
  await settings.getByRole('button', { name: /OpenRouter/ }).click()
  await expect(settings.getByRole('button', { name: 'openrouter/auto', exact: true })).toHaveClass(/selected/)
  await settings.getByText('Advanced connection').click()
  await settings.getByLabel('Custom model ID').fill('vendor/custom-model')
  await expect(settings.getByLabel('Custom model ID')).toHaveValue('vendor/custom-model')
