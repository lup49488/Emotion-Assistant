  ...
  return (
    <main className={`app-shell ${settingsOpen ? 'settings-open' : 'settings-closed'}`}>
      ...
      <aside className={`settings-panel ${settingsOpen ? 'open' : ''}`} aria-label="Chat preferences">
        ...
        <label className="model-field">{t('provider')}<select value={options.provider} onChange={(event) => setOptions((current) => ({ ...current, provider: event.target.value }))}><option value="">{t('serverDefault')}</option><option value="openai_compatible">OpenAI-compatible</option><option value="anthropic">Anthropic (Claude)</option><option value="deepseek">DeepSeek</option><option value="openai">OpenAI</option><option value="openrouter">OpenRouter</option><option value="custom">Custom endpoint</option></select></label>
