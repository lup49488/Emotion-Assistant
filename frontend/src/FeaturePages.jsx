...
export function KnowledgePage({ t, canManageKnowledge = false }) {
  ...
  const search = async (event) => { event.preventDefault(); if (!query.trim()) return; setError(''); try { setResults(await readJson('/api/v1/rag/search', { method: 'POST', headers: csrfHeaders(), body: JSON.stringify({ query, top_k: 4, threshold: 0.35, candidate_multiplier: 4, retrieval_mode: 'hybrid_rrf' }) })) } catch (requestError) { setError(requestError.message) } }
  ...
  const runEvaluation = async () => { await submitJob('/api/v1/rag/evaluations/run', { method: 'POST', headers: csrfHeaders(), body: JSON.stringify({ top_k: 4, threshold: 0.35, candidate_multiplier: 4, retrieval_mode: 'hybrid_rrf', compare_modes: true }) }) }
