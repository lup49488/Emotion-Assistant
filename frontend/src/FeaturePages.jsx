...
export function PrivacyPage({ onDeleted, t }) {
  ...
  return <section className="feature-page"><PageHeader title={t('privacyTitle')} description={t('privacyDescription')} action={<button className="secondary-button" onClick={refresh}><RefreshCw size={16} />{t('refresh')}</button>} /><ErrorText error={error} />{message && <p className="success-text">{message}</p>}
    ...
    <div className="two-column"><section className="data-panel"><h2>{t('exportTitle')}</h2><p>{t('exportDescription')}</p><button className="primary-button" onClick={exportData}><Download size={16} />{t('downloadExport')}</button></section><section className="data-panel"><h2>{t('importTitle')}</h2><p>{t('importDescription')}</p><form className="import-form" onSubmit={importData}><input type="file" accept="application/json,.json" aria-label={t('importFile')} onChange={(event) => setImportFile(event.target.files?.[0] || null)} /><label>{t('importMode')}<select value={importMode} onChange={(event) => setImportMode(event.target.value)}><option value="merge">{t('importMerge')}</option><option value="replace">{t('importReplace')}</option></select></label><button className="primary-button" disabled={!importFile}><Upload size={16} />{t('importData')}</button></form></section></div><section className="data-panel danger-panel"><h2><ShieldAlert size={18} />{t('deleteTitle')}</h2><p>{t('deleteDescription')}</p><input value={confirmation} placeholder={t('deletePlaceholder')} onChange={(event) => setConfirmation(event.target.value)} /><button className="danger-button" onClick={deleteData}><Trash2 size={16} />{t('deleteMyData')}</button></section>
  </section>
}
